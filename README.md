...
1.  **查看部署進度 (詳情畫面)**：
    *   點擊 GitHub 頂端的 **「Actions」** 分頁。
    *   在列表中點擊最新的那一次 **任務標題** (例如：「Update README...」)。
    *   點進去後即可看到視覺化的 **流程圖** (`build_site` -> `deploy`)，確認是否都顯示綠色勾勾。
2.  **專案首頁連結**：
    當部署成功後，GitHub 網頁右邊的 **「Deployments」** 面板會出現一個 **`github-pages`** 連結，點擊即可進入。
3.  **固定網址**：就是這一個 👇
    👉 **[https://derekchen6688.github.io/Pixel-Game/](https://derekchen6688.github.io/Pixel-Game/)**

> [!TIP]
> **為什麼打不開？**
> 請先檢查上方 **「Actions」** 分頁，必須看到名為 `Deploy to GitHub Pages` 的任務出現 **綠色勾勾 ✅**（代表編譯成功）後，連結才會生效。如果是黃色圈圈代表正在處理中，請耐心等候約 1 分鐘。

#### 4. 如何手動重新部署？ (Run workflow)
如果您修改了 **GitHub Secrets** (變數內容) 但沒有修改程式碼，或者部署過程中出錯想要重試：
1. 進入 GitHub 的 **「Actions」** 分頁。
2. 在左側點選 **「Deploy to GitHub Pages」** 流程。
3. 點擊右側的 **「Run workflow」** 下拉選單，然後按下綠色的 **「Run workflow」** 按鈕。
4. 這樣就能在不推送新程式碼的情況下，讓 GitHub 重新抓取最新設定並發布。
